...
- Расширенные категории метрик и дополнительные поля внедрены частично: добавлены `category`/`severity`, `reconnect_gap_ms` и `reconnect_total_attempts` на core-уровне, но без нативных полей из VPN layer.
