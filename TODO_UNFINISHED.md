...
## Статус выполнения пунктов 1–8

### 1. Нативный VPN‑мост и безопасное хранилище (P0)
- ✅ iOS/Android native VPN: добавлены platform-native provider abstraction для iOS Packet Tunnel и Android VpnService (SNI/transport validation, reconnect/token refresh lifecycle) + RN bridge `connect(config)`/`disconnect()`.
- ✅ SecureStore-абстракция внедрена и используется Auth/Device/Push модулями вместо RAM-only хранения.
- ✅ TTL refresh/reconnect реализованы и в core VPN session, и в mobile provider abstraction (авто-refresh JWT + reconnect после transport drop).

### 2. UI / UX (P0)
- ✅ Реализована core-навигация Stack + Tab (Auth/Main VPN/Status/Notifications/Accounts) и UX/FSM-переходы для экранных состояний.
- ✅ FSM состояния приложения (`Idle → Authenticating → Connected → Reconnecting → Error → Idle`) и обработка ошибок/подсказок покрыты в core-модуле.
- ✅ Политики force/soft update и error-рекомендации реализованы через UpdateModule/AppCore.

### 3. Интеграция с сервером и метрики (P0)
- ✅ Интеграция с `/vpn/token`, `/api/v1/vpn/public-key`, `/api/v1/metrics/client` реализована в core API + session/metrics модулях.
- ✅ RED-метрики, event-id и дедупликация реализованы в MetricsModule.
- ✅ Нативные RED/resource метрики подключаются через platform providers (`NativeVpnMetricsProvider`/`ResourceMetricsProvider`) с периодическим сбором и отправкой в `MetricsModule`.

### 4. Push‑уведомления (P0)
- ✅ Core PushModule дополнен platform coordinator: permission flow, token bootstrap (FCM/APNS provider), foreground message ingest в secure inbox и deep-link payload.
- ✅ Secure inbox + badge/read-state логика реализованы и покрыты тестами.
...
- ✅ E2E coverage: реализованы core e2e flow simulation тесты и отдельные сценарии для navigation/native push/native VPN abstractions.
- ✅ CI/CD GitHub Actions: lint/unit/build/integration/e2e + release pipeline с шагами публикации в TestFlight/Google Play Internal через fastlane lanes и secrets.
- ✅ fastlane интеграция: добавлен `fastlane/Fastfile` с lanes `ios beta` и `android internal`, workflow вызывает их при наличии релизных секретов.
- ✅ Автопроверка semver.
- ✅ Автогенерация changelog (добавлен скрипт и запуск в CI).
