...
### 1. Нативный VPN‑мост и безопасное хранилище (P0)
- ~ iOS/Android native VPN: реализован TypeScript bridge (`connect(config)`/`disconnect()`) и core tunnel lifecycle, но нативные Swift/Kotlin Network Extension/VpnService в этом репозитории не представлены.
- ✅ SecureStore-абстракция внедрена и используется Auth/Device/Push модулями вместо RAM-only хранения.
- ~ TTL refresh/reconnect реализованы на уровне core VPN session; полная нативная реализация transport/TLS зависит от мобильного репозитория.

### 2. UI / UX (P0)
- ~ Реализованы core UI-модели состояния (главный экран, статусы, UX FSM), но полноценная React Navigation + реальные RN-экраны не входят в scope репозитория.
...
- ~ Платформенно-точные метрики (ProcessInfo/ActivityManager/TrafficStats/BatteryStats) частично эмулированы в core; нативные сборщики требуют mobile-native layer.

### 4. Push‑уведомления (P0)
- ~ Core PushModule реализован (регистрация токена, inbox, read/delete, deep-link payload), но прямое подключение FCM/APNS SDK и UNUserNotificationCenter delegate вне scope текущего репозитория.
